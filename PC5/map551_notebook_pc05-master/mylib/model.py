...
class heat_model:
    ...
    def fcn(self, t, y):
        ...
        ydot = np.empty(nx)
        ydot[0] = oneondxdx * (-2*y[0] + 2*y[1])
        ydot[1:-1] = oneondxdx * (y[:-2] - 2*y[1:-1] + y[2:])
        ydot[nx-1] = oneondxdx * (2*y[nx-2] - 2*y[nx-1])
        return ydot
    ...
    def fcn_radau(self, n, t, y, f, rpar, ipar):
        ...
        f[0] = oneondxdx * (-2*y[0] + 2*y[1])
        for ix in range(1, nx-1):
            f[ix] = oneondxdx * (y[ix-1] - 2*y[ix] + y[ix+1])
        f[nx-1] = oneondxdx * (2*y[nx-2] - 2*y[nx-1])
    ...
    def fcn_rock(self, n, t, y, dy):
        ...
        dy[0] = oneondxdx * (-2*y[0] + 2*y[1])
        for ix in range(1, nx-1):
            dy[ix] = oneondxdx * (y[ix-1] - 2*y[ix] + y[ix+1])
        dy[nx-1] = oneondxdx * (2*y[nx-2] - 2*y[nx-1])
