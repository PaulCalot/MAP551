...
#############################################################################
class rock_result:
    def __init__(self, y, nfev, nstep, naccpt, nrejct, nfevrho, nstage, tsol=0):
        self.y = y
        self.nfev = nfev 
        self.nstep = nstep
        self.naccpt = naccpt
...
def rock4_dt(tini, tend, yini, fcn, tol):
    ...
    nfevrho = info[4]   # number of evaluations of f used to estimate the spectral radius
    nstage  = info[5]   # maximum number of stages used
    tsol = tsol[np.nonzero(tsol)]
    if tini == 0:
        tsol = np.insert(tsol, 0, tini)

    return rock_result(y, nfev, nstep, naccpt, nrejct, nfevrho, nstage, tsol)
