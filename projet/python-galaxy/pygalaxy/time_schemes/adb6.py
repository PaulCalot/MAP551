...
class ADB6: # lien : https://fr.wikipedia.org/wiki/M%C3%A9thodes_d%27Adams-Bashforth
    ...
    def init(self, mass, particles): # l'init réalise les 5 premiers pas nécessaires pour le calcul du 6 par Adams Bashforth (ordre 6)
        ...
        self.count_eval_force += self.method(mass, particles, self.f[5], **self.args_method)
        self.count += 1 + rk4.count
        self.count_eval_force += rk4.count_eval_force
    ...
    def update(self, mass, particles):
        ...
        self.count_eval_force += self.method(mass, particles, self.f[5], **self.args_method) # method est ici l'énergie (dans le cas de l'exemple fournit)
        self.count += 1
